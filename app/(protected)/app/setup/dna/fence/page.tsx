...
import { CompanyDnaFenceIntro } from "@/components/company-dna/CompanyDnaFenceIntro";
...
export default async function CompanyDnaFenceLandingPage({
  searchParams,
}: PageProps) {
  ...
  const area = hub.progress.find((item) => item.workAreaType === "fence");
  const calibratedKeys =
    area?.tasks
      .filter((status) => status.calibrated)
      .map((status) => status.calibrationTaskKey) ?? [];
  const counts = fenceV2ProgressCounts(calibratedKeys);
  const nextTask = nextCompanyDnaV2Task({
    workAreaType: "fence",
    calibratedTaskKeys: calibratedKeys,
  });
  const firstTask = listCompanyDnaFenceV2UiTasks()[0];
  const firstHref = `/app/setup/dna/${encodeURIComponent(
    nextTask?.calibrationTaskKey ?? firstTask?.calibrationTaskKey ?? "fence.posts.v1"
  )}`;
