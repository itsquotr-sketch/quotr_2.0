...
import { CompanyDnaBathroomIntro } from "@/components/company-dna/CompanyDnaBathroomIntro";
...
export default async function CompanyDnaBathroomLandingPage({
  searchParams,
}: PageProps) {
  ...
  const area = hub.progress.find((item) => item.workAreaType === "bathroom");
  const calibratedKeys =
    area?.tasks
      .filter((status) => status.calibrated)
      .map((status) => status.calibrationTaskKey) ?? [];
  const counts = bathroomV2ProgressCounts(calibratedKeys);
  const nextTask = nextCompanyDnaV2Task({
    workAreaType: "bathroom",
    calibratedTaskKeys: calibratedKeys,
  });
  const firstTask = listCompanyDnaBathroomV2UiTasks()[0];
  const firstHref = `/app/setup/dna/${encodeURIComponent(
    nextTask?.calibrationTaskKey ?? firstTask?.calibrationTaskKey ?? "bathroom.lining.wall.v1"
  )}`;
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      <PageHeader
        title="Bathroom calibration"
        description="Tell Quotr how your crew normally completes common bathroom tasks."
        actions={
