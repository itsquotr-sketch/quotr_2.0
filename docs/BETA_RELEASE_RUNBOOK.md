...
Preview today: `shhpjsoldmqtkdbgrbtm`, migration **052**, Stripe **TEST**, `BILLING_ENVIRONMENT=test`.  
Production today: `lxvnylhsbvudzzupxeqr`, migration **045**. **Do not deploy. Do not migrate.**

There is **no migration 037** (001–036, 038–052). That gap is historical and documented.

Related: `docs/BETA_LAUNCH_CHECKLIST.md`, `docs/BETA_SMOKE_TEST.md`, `docs/runbooks/MIGRATION_053_ROLE_RLS_PROPOSAL.md`, `docs/architecture/QUOTR_ENVIRONMENT_ARCHITECTURE.md`.

---

## 0. Owner gates (must all be yes)

1. 053 RLS proposal accepted or explicitly deferred with Estimator-invite ban
...
## 2. Production migration sequence (046 → 052)
...
See proposal. Apply **after** 052, still Preview-first, then Production. Not created in this programme.

### Rehearsal

Preview already contains 046–052. That is the coherence proof. Do not clone Production into Preview. Static chain: 046 additive → 047 column → 048 config/provision → 049 memberships → 050 unbind → 051 tz → 052 DNA using memberships.
