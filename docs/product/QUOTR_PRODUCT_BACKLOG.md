...
**Status:** Active  
**Created:** 2026-08-05  
**Governing stage:** Stage 3.1A **Complete**; Stage 3.1D **Complete**; Stage 3.1B **In Progress** (3.1B.1–3.1B.5A Complete — Local; 3.1B.5B Not Started)  
...
**Constraint:** Items marked Deferred must not be implemented until authorised; deferred schema proposals remain **Not Approved**; migrations 028/029 Complete — Local, Not Applied Remotely; AI provider adapter Implemented but unused; UI / production catalogue adoption Not Started; Analyse Job Unchanged  
...
| ISD-001 | Scope suggestion lifecycle | Feature | P0 | No canonical propose→decide→apply contract across Analyse Job vs notes | Suggested WAs written immediately on Analyse Job; notes use `note_proposals` | Canonical `ScopeDiscoverySuggestion` statuses/transitions; AI proposes; user decides | Stage 3.1B.1 / 3.1B.5A | Boundary + suggestion contract; Fact SoT | Complete — Local (contract + local accept/reject/modify RPCs; no production adoption) | `verify-stage-3-1b1-suggestion-contract.ts`; `verify-stage-3-1b5a-decision-lifecycle.ts` | Rejection/modification provenance for Evidence Engine | Production wiring remains 3.1B.5B |
...
| ISD-006 | Discovery rerun / idempotency | Feature | P0 | Analyse Job one-shot; no stale/supersede; duplicate risk on notes | Brief locked; note analyse blocked while pending | ScopeDiscoveryRun idempotency; stale/supersede; no duplicate provider calls | Stage 3.1B.4A / 3.1B.4B | OCD-ISD-06/07/08; persistence owner approval | Complete — Local (orchestration + migration 028 local); remote apply Not Approved | `verify-stage-3-1b4a-discovery-orchestration.ts`; `verify-stage-3-1b4b-persistence.ts` | Run history for learning later | Explicit user trigger; Analyse Job unchanged |
...
- Stage 3.1B.5B production wiring: **Not Started**.
- Migrations: `028`/`029` local only; UI **Not Started**; production adoption **Not Started**.
