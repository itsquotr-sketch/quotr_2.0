...
**Status:** Active  
**Created:** 2026-08-05  
**Governing stage:** Stage 3.1A **Complete**; Stage 3.1D **Complete**; Stage 3.1B **In Progress** (3.1B.1–3.1B.5A Complete — Local; 3.1B.5B Complete — Planning)  
...
**Constraint:** Items marked Deferred must not be implemented until authorised; migrations 028/029 Complete — Local, remote apply **Ready Pending Owner Approval**; production wiring Ready Pending Remote Migration; feature flag Planned; AI provider adapter Implemented but unused; UI / production catalogue adoption Not Started; Analyse Job Unchanged  
...
| ISD-001 | Scope suggestion lifecycle | Feature | P0 | No canonical propose→decide→apply contract across Analyse Job vs notes | Suggested WAs written immediately on Analyse Job; notes use `note_proposals` | Canonical `ScopeDiscoverySuggestion` statuses/transitions; AI proposes; user decides | Stage 3.1B.1 / 3.1B.5A / 3.1B.5B | Boundary + suggestion contract; Fact SoT | Complete — Local (contract + RPCs); 3.1B.5B wiring design Complete — Planning; production adoption Not Started | `verify-stage-3-1b1-suggestion-contract.ts`; `verify-stage-3-1b5a-decision-lifecycle.ts`; wiring design + remote runbook | Rejection/modification provenance for Evidence Engine | Remote apply + server actions owner-gated |
...
| ISD-006 | Discovery rerun / idempotency | Feature | P0 | Analyse Job one-shot; no stale/supersede; duplicate risk on notes | Brief locked; note analyse blocked while pending | ScopeDiscoveryRun idempotency; stale/supersede; no duplicate provider calls | Stage 3.1B.4A / 3.1B.4B / 3.1B.5B | OCD-ISD-06/07/08; persistence owner approval | Complete — Local; remote apply Ready Pending Owner Approval (3.1B.5B) | `verify-stage-3-1b4a-discovery-orchestration.ts`; `verify-stage-3-1b4b-persistence.ts`; remote runbook | Run history for learning later | Explicit user trigger; Analyse Job unchanged |
...
- Stage 3.1B.4B persistence: **Complete — Local** — migration `028`; `lib/scope-discovery/persistence/*`; **Not Applied Remotely**; unused by Analyse Job.
- Stage 3.1B.5A decision lifecycle: **Complete — Local** — migration `029`; `lib/scope-discovery/decisions/*`; **Not Applied Remotely**; unused by UI/Analyse Job.
- Stage 3.1B.5B remote readiness + wiring design: **Complete — Planning** — runbook, wiring design, Preview rollout, approval register; remote apply **Not Approved**; no production code/UI.
- Migrations: `028`/`029` Ready Pending Owner Approval; production wiring Ready Pending Remote Migration; feature flag **Planned**; UI **Not Started**; Analyse Job **Unchanged**.
