...
**Status:** Active  
**Created:** 2026-08-05  
**Governing plan:** `docs/plans/QUOTR_DEVELOPMENT_MASTER_PLAN.md`. **Governing stage:** … **PHASE 0 COMPLETE / ARCHITECTURE FROZEN.** REQ-1 **COMPLETE / TECHNICALLY VALIDATED**. REQ-2 **COMPLETE / MATERIAL EMISSION FOUNDATION VALIDATED**. REQ-2.1 **COMPLETE / TECHNICALLY VALIDATED**. REQ-3 **COMPLETE / LABOUR EMISSION FOUNDATION VALIDATED**. REQ-3.1 **COMPLETE / TECHNICALLY VALIDATED**. REQ-4 **COMPLETE / COMPONENT AUTHORITY MIGRATION VALIDATED**. REQ-4A **COMPLETE / TECHNICALLY VALIDATED**. REQ-4B **COMPLETE / REMOTE VALIDATED** (`f99dbec`). REQ-SNAPSHOT-01 **COMPLETE / REMOTE VALIDATED**. REQ-TXN-01 **COMPLETE / REMOTE VALIDATED**. **DECK-1 READY / NOT STARTED**. **DECK-1A COMPLETE LOCAL / OWNER MODEL REVIEW PENDING**. CM-03 **BACKLOG / NOT STARTED**.
...
| CM-03 | Labour benchmark-disabled fallback source-label mismatch | Commercial correctness | P2 | Resolver may show “Pricing required” while labour is still priced at $60/$90 | `allow_benchmark_rates=false` + no company labour rate still injects 60/90; `sourceType` may be `missing` | Clarify product semantics and resolver provenance without accidentally removing the live fallback money | Post REQ-3 / commercial rates | REQ-3.1 R1; `resolveLabourRate`; COMMERCIAL-P0 CF-D2 | **Backlog / Not Started** | Prove label matches money, or prove unpriced labour if that becomes the product rule; goldens must not silently change | Rate provenance is DNA evidence later | Do not “fix” by omitting 60/90 in REQ-3/REQ-4 |
