...
**Mode:** PHASE 0 **COMPLETE / ARCHITECTURE FROZEN** after independent challenge (PHASE 0-R1). Does not start REQ-1.  
...
REQ-1 remains the immediate implementation batch after this freeze. Independent review: **GO**. Remaining Owner Previews (Rates, Branding, DEMO-R7, optional R2-R1 Deck $ check) are polish, not architecture start-blockers.
...
| **Priority** | CRITICAL |
| **Status** | REQ-1 **READY / NOT STARTED**; REQ-2/3/4 **NOT STARTED** |
| **Batches** | **REQ-1** envelope + physical aggregation; **REQ-2** MaterialRequirement emission (Deck first); **REQ-3** LabourRequirement emission; **REQ-4** requirement ↔ pricing reconciliation / shadow / promotion |
...
1. PHASE 0 is frozen. No further broad architecture-review loop unless implementation reveals a genuine defect.  
2. Next implementation batch: **REQ-1**.  
3. Do not start REQ-1 in this PHASE 0-R1 task.  
