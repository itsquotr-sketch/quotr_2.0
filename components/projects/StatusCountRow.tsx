...
export function StatusCountRow({
  ...
  return (
    <div
      className={cn(
        "grid grid-cols-2 gap-1.5 sm:grid-cols-3 sm:gap-2 lg:grid-cols-6",
        className
      )}
    >
      {STATUS_ITEMS.map((item) => {
        ...
        return (
          <Link
            ...
            className={cn(
              "min-h-11 rounded-lg border px-2.5 py-1.5 transition-[border-color,background-color,box-shadow] sm:min-h-0 sm:px-3 sm:py-2.5",
              isActive
