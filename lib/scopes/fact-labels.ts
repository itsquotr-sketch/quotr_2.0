import { factHasValue, normalizeBooleanForUi } from "@/lib/scopes/fact-values";

/** Free-text / boolean tokens that mean “not answered with certainty”. */
const NOT_SURE_VALUES = new Set([
  "not sure",
  "not_sure",
  "unknown",
  "unsure",
]);

/** Tokens that remain “not sure” even when listed as select options. */
const EXPLICIT_NOT_SURE_OPTIONS = new Set([
  "not sure",
  "not_sure",
  "unsure",
]);

/** Maps alternate fact keys to a canonical key for deduplication. */
const FACT_KEY_ALIASES: Record<string, string> = {
  "deck.decking_material": "deck.board_material",
  "deck.material": "deck.board_material",
  "deck.stairs": "deck.access_type",
  "deck.has_stairs": "deck.access_type",
  "deck.balustrade": "deck.balustrade_required",
  "deck.has_balustrade": "deck.balustrade_required",
  "deck.demolition_required": "deck.existing_deck_removal",
  "fence.has_gate": "fence.gate_included",
  "fence.gate": "fence.gate_included",
  "pergola.attachment": "pergola.attached",
  "bathroom.waterproofing_required": "bathroom.waterproofing_included",
  "bathroom.waterproofing_scope": "bathroom.waterproofing_extent",
  "bathroom.plumbing_allowance": "bathroom.plumbing.level",
  "bathroom.electrical_allowance": "bathroom.electrical.level",
  "bathroom.plumbing_changes": "bathroom.plumbing.level",
  "bathroom.electrical_changes": "bathroom.electrical.level",
  "demolition.waste_removal_required": "demolition.disposal_included",
  "external_stairs.riser_count": "external_stairs.risers_count",
  "external_stairs.handrail_required": "external_stairs.handrail_included",
  "retaining_wall.high_height_m": "retaining_wall.height_high_m",
  "retaining_wall.low_height_m": "retaining_wall.height_low_m",
  "kitchen.plumbing_required": "kitchen.plumbing_changes",
  "kitchen.electrical_required": "kitchen.electrical_changes",
  "kitchen.has_island": "kitchen.island_included",
  "fence.post_centres_m": "fence.post_spacing_m",
  "fence.panel_type": "fence.paling_or_panel_type",
  "doors.door_count": "doors.count",
  "internal_walls.length_m": "internal_walls.length_lm",
  "painting.coats": "painting.coats_required",
  "painting.client_supplied": "painting.paint_client_supplied",
  "flooring.supply_by": "flooring.supply_scope",
  "plastering.finish_level": "plastering.level",
  "demolition.hazardous_materials_suspected": "demolition.hazardous_materials_risk",
  "demolition.services_isolation_required": "demolition.services_isolated",
};

/** Display labels for canonical fact keys. */
const FACT_DISPLAY_LABELS: Record<string, string> = {
  "deck.length_m": "Length",
  "deck.width_m": "Width",
  "deck.area_m2": "Area",
  "deck.material": "Decking material",
  "deck.level": "Deck level",
  "deck.demolition_required": "Existing deck removal",
  "deck.has_stairs": "Stairs",
  "deck.has_balustrade": "Balustrade",
  "deck.pergola_included": "Pergola",
  "deck.board_direction": "Board direction",
  "deck.joist_direction": "Joist direction",
  "deck.joist_section": "Joist size",
  "deck.joist_centres_mm": "Joist spacing",
  "deck.framing_treatment": "Framing treatment",
  "deck.bearer_section": "Bearer size",
  "deck.bearer_row_count": "Bearer rows",
  "deck.support_type": "Support type",
  "deck.supports_per_bearer": "Supports per bearer",
  "deck.support_section": "Support size",
  "deck.footing_length_mm": "Footing length",
  "deck.footing_width_mm": "Footing width",
  "deck.footing_depth_mm": "Footing depth",
  "pergola.area_m2": "Area",
  "pergola.material": "Material",
  "pergola.roofing_included": "Roofing/covering",
  "pergola.attached": "Attachment",
  "fence.has_gate": "Gate",
  "fence.demolition_required": "Existing fence removal",
  "fence.access": "Access",
  "fence.slope_condition": "Ground slope",
  "pergola.length_m": "Length",
  "pergola.width_m": "Width",
  "pergola.roofing_type": "Roofing type",
  "pergola.gutters_included": "Gutters",
  "deck.access": "Access",
  "deck.handrail_required": "Handrail",
  "retaining_wall.length_m": "Wall length",
  "retaining_wall.height_m": "Wall height",
  "retaining_wall.height_high_m": "High point",
  "retaining_wall.height_low_m": "Low point",
  "retaining_wall.average_height_m": "Average height",
  "retaining_wall.material": "Wall material",
  "retaining_wall.drainage_required": "Drainage",
  "retaining_wall.excavation_required": "Excavation",
  "retaining_wall.backfill_included": "Backfill",
  "retaining_wall.carting_distance_m": "Carting distance",
  "retaining_wall.disposal_included": "Spoil removal",
  "retaining_wall.spoil_removal_portion": "Spoil leaving site",
  "retaining_wall.spoil_removal_volume_m3": "Spoil removal volume",
  "retaining_wall.access": "Access",
  "deck.pile_or_post_replacement_required": "Pile/post replacement",
  "deck.pile_or_post_count": "Piles/posts count",
  "deck.substructure_condition": "Substructure condition",
  "deck.steps_included": "Steps",
  "deck.concrete_to_supports": "Concrete to supports",
  "deck.concrete_bags_per_hole": "Concrete bags per hole",
  "fence.finish_required": "Fence finish",
  "fence.finish_type": "Finish type",
  "fence.finish_sides": "Finish sides",
  "pergola.finish_required": "Pergola finish",
  "pergola.finish_type": "Pergola finish type",
  "bathroom.area_m2": "Area",
  "bathroom.length_m": "Length",
  "bathroom.width_m": "Width",
  "bathroom.wall_height_m": "Wall height",
  "bathroom.floor_area_m2": "Floor area",
  "bathroom.ceiling_area_m2": "Ceiling area",
  "bathroom.gross_wall_area_m2": "Wall area",
  "bathroom.job_scope": "Bathroom work",
  "bathroom.floor_finish_system": "Floor finish",
  "bathroom.floor_substrate_system": "Floor build-up",
  "bathroom.floor_substrate_sheet_size": "Fibre-cement flooring size",
  "bathroom.wall_lining_included": "Wall lining",
  "bathroom.ceiling_lining_included": "Ceiling lining",
  "bathroom.framing_level": "Framing / nogging",
  "bathroom.plumbing.level": "Plumbing",
  "bathroom.electrical.level": "Electrical",
  "bathroom.demolition_required": "Demolition",
  "bathroom.demolition.components": "What is being stripped out",
  "bathroom.waste.level": "Disposal allowance",
  "bathroom.stopping_included": "Stopping / plastering",
  "bathroom.painting_included": "Painting",
  "bathroom.waterproofing_required": "Waterproofing",
  "bathroom.tile_extent": "Wall tiling",
  "bathroom.tile_format": "Tile format",
  "bathroom.shower.width_m": "Shower width",
  "bathroom.shower.depth_m": "Shower depth",
  "bathroom.shower.wall_height_m": "Shower wall height",
  "bathroom.fixtures_included": "Fixtures",
  "bathroom.fixtures_client_supplied": "Client-supplied fixtures",
  "bathroom.plumbing.scope_text": "Plumbing scope notes",
  "bathroom.electrical.scope_text": "Electrical scope notes",
  "bathroom.electrical.light_count": "Light points",
  "bathroom.electrical.gpo_count": "GPOs",
  "bathroom.electrical.mirror_power_included": "Mirror or vanity power",
  "bathroom.electrical.new_circuit_included": "New circuit",
  "bathroom.plumbing.floor_waste_included": "Floor waste",
  "bathroom.plumbing.relocation_count": "Fixture relocations",
  "bathroom.fixture.vanity.ownership": "Vanity supply / install",
  "bathroom.fixture.toilet.ownership": "Toilet supply / install",
  "bathroom.fixture.mirror.ownership": "Mirror supply / install",
  "bathroom.includes_shower": "Shower",
  "bathroom.includes_toilet": "Toilet",
  "bathroom.finish_level": "Finish level",
  "kitchen.area_m2": "Area",
  "kitchen.demolition_required": "Demolition",
  "kitchen.plumbing_required": "Plumbing",
  "kitchen.electrical_required": "Electrical",
  "kitchen.cabinetry_included": "Cabinetry",
  "kitchen.benchtop_included": "Benchtop",
  "kitchen.benchtop_material": "Benchtop material",
  "kitchen.island_included": "Island",
  "kitchen.island_length_m": "Island length",
  "kitchen.cabinetry_lm": "Cabinetry length",
  "kitchen.finish_level": "Finish level",
  "bathroom.waterproofing_extent": "Waterproofing extent",
  "retaining_wall.post_spacing_m": "Post spacing",
  "pergola.height_m": "Height",
  "internal_walls.fire_or_acoustic": "Fire or acoustic rating",
  "internal_walls.job_scope": "Wall work",
  "internal_walls.structural_involvement": "Load-bearing or structural",
  "internal_walls.wall_type.frame_system": "Framing",
  "internal_walls.wall_type.frame_size": "Timber framing",
  "internal_walls.wall_type.wall_count": "Physical wall count",
  "internal_walls.wall_type.length_lm": "Total wall length",
  "internal_walls.wall_type.height_m": "Wall height",
  "internal_walls.wall_type.stud_centres_mm": "Stud spacing",
  "internal_walls.wall_type.side_a_product": "Side A lining",
  "internal_walls.wall_type.side_b_product": "Side B lining",
  "internal_walls.wall_type.same_lining_both_sides": "Same lining both sides",
  "internal_walls.wall_type.has_openings": "Openings",
  "internal_walls.wall_type.insulation_included": "Wall insulation",
  "internal_walls.wall_type.insulation": "Insulation type",
  "internal_walls.wall_type.skirting": "Skirting",
  "internal_walls.wall_type.cornice": "Cornice",
  "internal_walls.wall_type.electrical": "Electrical",
  "internal_walls.wall_type.electrical_note": "Electrical notes",
  "internal_walls.wall_type.stopping_side_a": "Stopping — Side A",
  "internal_walls.wall_type.stopping_side_b": "Stopping — Side B",
  "internal_walls.wall_type.painting": "Wall painting",
  "internal_walls.opening.type": "Opening type",
  "internal_walls.opening.width_m": "Opening width",
  "internal_walls.opening.height_m": "Opening height",
  "fence.post_spacing_m": "Post spacing",
  "fence.paling_or_panel_type": "Paling or panel type",
  "fence.gate_width_m": "Gate width",
  "fence.system": "Fence type",
  "fence.timber_species": "Timber species",
  "fence.board_thickness_mm": "Board thickness",
  "fence.top_capping": "Top capping",
  "fence.slat_gap_mm": "Slat gap",
  "fence.post_embedment_m": "Post embedment",
  "fence.hole_diameter_m": "Post-hole diameter",
  "fence.section_width_m": "Section width",
  "fence.section_count": "Section count",
  "fence.metal_material": "Metal type",
  "fence.gate_position": "Gate position",
  "fence.gate_capping": "Gate matches fence capping",
  "fence.horizontal_course_count": "Horizontal slat course count",
  "fence.vertical_paling_gap_mm": "Gap between vertical palings",
  "fence.rail_section": "Fence rail section",
  "ceilings.area_m2": "Ceiling area",
  "ceilings.structure_type": "Ceiling structure",
  "ceilings.ceiling_type": "Ceiling lining",
  "ceilings.plasterboard_type": "Plasterboard type",
  "ceilings.portion.geometry_mode": "Geometry",
  "ceilings.portion.length_m": "Ceiling length",
  "ceilings.portion.width_m": "Ceiling width",
  "ceilings.portion.area_m2": "Ceiling area",
  "ceilings.portion.job_scope": "Ceiling work",
  "ceilings.portion.structure_family": "Ceiling structure",
  "ceilings.portion.structure_requirements": "New framing",
  "ceilings.portion.timber_size": "Timber size",
  "ceilings.portion.spacing_mm": "Framing spacing",
  "ceilings.portion.direction": "Run direction",
  "ceilings.portion.lining_family": "Ceiling lining",
  "ceilings.portion.plasterboard_product": "Plasterboard type",
  "ceilings.portion.thickness_mm": "Plasterboard thickness",
  "ceilings.bulkhead.thickness_mm": "Bulkhead plasterboard thickness",
  "ceilings.portion.sheet_length_mm": "Sheet length",
  "ceilings.portion.sheet_width_mm": "Sheet width",
  "ceilings.portion.layers": "Lining layers",
  "ceilings.portion.height_m": "Ceiling height",
  "ceilings.portion.insulation_included": "Ceiling insulation",
  "ceilings.portion.bulkheads_present": "Bulkheads",
  "ceilings.portion.demolition_included": "Ceiling demolition",
  "ceilings.portion.stopping_included": "Ceiling stopping",
  "ceilings.portion.painting_included": "Ceiling painting",
  "ceilings.portion.penetrations": "Penetrations",
  "ceilings.portion.fire_acoustic_system": "Fire or acoustic system",
  "ceilings.portion.drop_height_m": "Drop height",
  "ceilings.portion.suspension_spacing_m": "Suspension spacing",
  "ceilings.portion.edge_offset_m": "Edge offset",
  "ceilings.bulkhead.length_m": "Bulkhead length",
  "ceilings.bulkhead.depth_m": "Bulkhead depth",
  "ceilings.bulkhead.height_m": "Bulkhead height",
  "ceilings.bulkhead.framing_type": "Bulkhead framing",
  "ceilings.bulkhead.lining_type": "Bulkhead lining",
  "ceilings.bulkhead.topology": "Bulkhead shape",
  "doors.portions": "Door sets",
  "doors.active_portion_id": "Selected door set",
  "doors.portion.label": "Door set location",
  "doors.portion.installation_type": "Installation type",
  "doors.portion.leaf_construction": "Leaf construction",
  "doors.portion.height_mm": "Door height",
  "doors.portion.width_mm": "Door width",
  "doors.portion.quantity": "Door quantity",
  "doors.portion.hardware_included": "Hardware included",
  "doors.portion.other_description": "Door description",
  "flooring.portions": "Flooring areas",
  "flooring.active_portion_id": "Selected flooring area",
  "flooring.portion.label": "Flooring area location",
  "flooring.portion.finish_type": "Floor finish",
  "flooring.portion.area_input_method": "Area method",
  "flooring.portion.length_m": "Floor length",
  "flooring.portion.width_m": "Floor width",
  "flooring.portion.area_m2": "Floor area",
  "flooring.portion.underlay_required": "Carpet underlay",
  "flooring.portion.floor_preparation_required": "Floor preparation",
  "flooring.portion.tile_width_mm": "Tile width",
  "flooring.portion.tile_length_mm": "Tile length",
  "flooring.portion.hardwood_board_width_mm": "Board width",
  "flooring.portion.substrate_required": "New substrate",
  "flooring.portion.substrate_family": "Substrate family",
  "flooring.portion.substrate_item_key": "Substrate product",
  "flooring.portion.framing_required": "New framing",
  "flooring.portion.framing_allowance_level": "Framing allowance",
  "flooring.portion.finish_removal_required": "Existing finish removal",
  "flooring.portion.existing_finish_type": "Existing floor finish",
  "flooring.portion.substrate_removal_required": "Substrate removal",
  "flooring.portion.other_description": "Flooring description",
  "flooring.portion.specialist_kind": "Specialist flooring",
  "cladding.portions": "Cladding sections",
  "cladding.active_portion_id": "Selected cladding section",
  "cladding.portion.label": "Cladding section location",
  "cladding.portion.scope_intent": "Cladding scope",
  "cladding.portion.cladding_family": "Cladding family",
  "cladding.portion.cladding_system": "Cladding system",
  "cladding.portion.orientation": "Cladding orientation",
  "cladding.portion.approved_profile": "Cladding profile",
  "cladding.portion.nominal_width_mm": "Nominal width",
  "cladding.portion.nominal_thickness_mm": "Nominal thickness",
  "cladding.portion.effective_cover_mm": "Effective cover",
  "cladding.portion.board_sheet_length_mm": "Sheet length",
  "cladding.portion.board_sheet_width_mm": "Sheet width",
  "cladding.portion.board_gap_mm": "Vertical sheet gap",
  "cladding.portion.batten_width_mm": "Batten width",
  "cladding.portion.batten_thickness_mm": "Batten thickness",
  "cladding.portion.area_method": "Area method",
  "cladding.portion.direct_area_m2": "Cladding area",
  "cladding.portion.length_m": "Cladding length",
  "cladding.portion.height_m": "Cladding height",
  "cladding.portion.openings_already_deducted": "Openings already deducted",
  "cladding.portion.opening_area_m2": "Opening area",
  "cladding.portion.cavity_included": "Cavity battens",
  "cladding.portion.wall_underlay_or_rab_included": "Wall underlay or rigid air barrier",
  "cladding.portion.trims_flashings_corners_included": "Trims, flashings and corners",
  "cladding.portion.existing_cladding_removal_required": "Existing cladding removal",
  "cladding.portion.painting_or_coating_included": "Painting or coating",
  "cladding.portion.specialist_kind": "Specialist cladding",
  "cladding.portion.other_description": "Cladding description",
};

export function getCanonicalFactKey(
  rawKey: string,
  workAreaType: string
): string {
  const withPrefix = rawKey.includes(".")
    ? rawKey
    : `${workAreaType}.${rawKey}`;
  return FACT_KEY_ALIASES[withPrefix] ?? withPrefix;
}

export function getFactDisplayLabel(
  canonicalKey: string,
  fallbackLabel?: string | null
): string {
  if (FACT_DISPLAY_LABELS[canonicalKey]) {
    return FACT_DISPLAY_LABELS[canonicalKey];
  }
  if (fallbackLabel?.trim()) {
    return fallbackLabel.trim();
  }
  const suffix = canonicalKey.includes(".")
    ? canonicalKey.split(".").pop()!
    : canonicalKey;
  return formatSnakeCase(suffix);
}

export function getDerivedHeightLabel(canonicalKey: string): string | null {
  if (canonicalKey === "retaining_wall.height_m") {
    return "Average height";
  }
  return null;
}

function formatSnakeCase(value: string): string {
  return value
    .split("_")
    .filter(Boolean)
    .map((word, index) => {
      const lower = word.toLowerCase();
      if (index > 0 && (lower === "m" || lower === "m2")) {
        return lower === "m2" ? "m²" : lower;
      }
      if (lower === "m2") return "m²";
      return lower.charAt(0).toUpperCase() + lower.slice(1);
    })
    .join(" ")
    .replace(/ m²/g, " m²")
    .replace(/\s+m$/g, " m");
}

function titleCaseSafe(value: string): string {
  if (value.includes("_")) {
    return formatSnakeCase(value);
  }
  if (value === value.toUpperCase() && value.length <= 4) {
    return value;
  }
  return value
    .split(/[\s-]+/)
    .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
    .join(" ");
}

/**
 * Whether a stored value means “not sure / unanswered with certainty”.
 *
 * When `selectOptions` is provided, a value that is an explicit listed option
 * is treated as a deliberate answer unless it is an explicit not-sure token
 * (`Not sure`, `not_sure`, `unsure`). Listed `"unknown"` / `"none"` therefore
 * count as answered for select questions (BUG-001 / UX-002).
 */
export function isNotSureValue(
  value: unknown,
  selectOptions?: string[]
): boolean {
  if (typeof value !== "string") return false;
  const trimmed = value.trim();
  const lower = trimmed.toLowerCase();

  if (selectOptions && selectOptions.length > 0) {
    const isListed = selectOptions.some(
      (option) => option.trim().toLowerCase() === lower
    );
    if (isListed) {
      return (
        EXPLICIT_NOT_SURE_OPTIONS.has(lower) || trimmed === "Not sure"
      );
    }
  }

  return NOT_SURE_VALUES.has(lower) || trimmed === "Not sure";
}

const ENUM_ANSWER_LABELS: Record<string, string> = {
  good_existing: "Good existing",
  good_condition: "Good condition",
  partial_replacement: "Partial replacement",
  full_replacement: "Full replacement",
  unknown: "Unknown",
  none: "None",
  not_required: "Not required",
  required: "Required",
  one_side: "One side",
  both_sides: "Both sides",
  clear_coat: "Clear coat",
  paint: "Paint",
  stain: "Stain",
  treated_plywood: "19 mm treated plywood",
  fibre_cement: "18 mm fibre cement",
  fibre_cement_flooring_19mm: "19 mm fibre-cement flooring",
  secura_flooring: "Secura flooring",
  floor_finish: "Floor finish",
  wall_lining: "Wall lining",
  ceiling: "Ceiling lining",
  vanity: "Vanity",
  toilet: "Toilet",
  shower: "Shower / enclosure",
  bath: "Bath",
  fixture: "Other fixtures",
  minor: "Minor",
  standard: "Standard",
  major: "Major",
  thermal: "Thermal / standard ceiling insulation",
  acoustic: "Acoustic insulation",
  thermal_acoustic: "Combination thermal + acoustic",
  existing_specified: "Existing specified product",
  other: "Other / custom",
};

/**
 * Presentation-only label for stored enum / select option values.
 * Does not mutate persisted values.
 */
export function formatAnswerOptionLabel(value: unknown): string {
  if (value === null || value === undefined || value === "") return "—";
  const raw = String(value);
  const mapped =
    ENUM_ANSWER_LABELS[raw] ?? ENUM_ANSWER_LABELS[raw.toLowerCase()];
  if (mapped) return mapped;
  if (raw.includes("_")) return formatSnakeCase(raw);
  return titleCaseSafe(raw);
}

export function formatSelectAnswerValue(value: unknown): string {
  return formatAnswerOptionLabel(value);
}

export function formatFactValueForDisplay(
  value: unknown,
  unit?: string | null,
  selectOptions?: string[]
): string | null {
  if (!factHasValue(value)) return null;

  if (Array.isArray(value)) {
    const parts = value
      .map((item) => formatAnswerOptionLabel(item))
      .filter((item) => item && item !== "—");
    return parts.length > 0 ? parts.join(", ") : null;
  }

  const boolLabel = normalizeBooleanForUi(value);
  if (boolLabel) return boolLabel;

  if (typeof value === "number") {
    const formatted = Number.isInteger(value) ? String(value) : String(value);
    return unit ? `${formatted} ${unit}` : formatted;
  }

  if (typeof value === "string") {
    if (isNotSureValue(value, selectOptions)) return "Not sure";
    return formatAnswerOptionLabel(value);
  }

  return String(value);
}
