...
import { DEFAULT_ANALYSIS_OBJECTIVE, type GetResultsInput, type ResultsReadOutcome } from "./types";
...
export async function getScopeDiscoveryResults(
  ...
  if (!availability.featureEnabled && !deps.allowReadWhenDisabled) {
    return {
      ...
      suppressedCount: 0,
      warnings: [],
      message: "Scope discovery is not enabled.",
    };
  }
  ...
  if (!run) {
    return {
      ...
      suppressedCount: 0,
      warnings: [],
      message: "No scope discovery results yet.",
    };
  }
